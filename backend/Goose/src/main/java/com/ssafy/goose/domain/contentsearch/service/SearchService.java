...
public class SearchService {
    ...
    @Autowired
    public SearchService(ContentNewsRepository contentNewsRepository, InternetSearchService internetSearchService, NewsAgencyExtractor newsAgencyExtractor) {
    ...
    public List<NewsResponseDto> searchNewsByKeyword(String[] keywords) {
        ...
        for (NewsResponseDto news : newsResults) {
            if (news.getNewsAgency() == null || news.getNewsAgency().equals("Unknown")) {
                String extractedAgency = newsAgencyExtractor.extractNewsAgency(news.getOriginalLink());
    ...
    private List<NewsResponseDto> convertNewsToNewsResponse(List<News> newsList) {
        ...
        for (News news : newsList) {
            ...
            if (newsAgency == null || newsAgency.equals("Unknown")) {
                newsAgency = newsAgencyExtractor.extractNewsAgency(news.getOriginalLink());
            }
            ...
            NewsResponseDto newsDto = NewsResponseDto.builder()
                    ...
                    .newsAgency(newsAgency) // 변경된 newsAgency 반영
